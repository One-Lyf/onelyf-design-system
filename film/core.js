...
const TAU = Math.PI * 2;
// v2: the sign-off is set in the brand serif (Fraunces) so OneLyf Title Case reads unmistakably;
// the handwriting faces rendered the capital O as a lowercase o.
const HAND_FONT = '"Fraunces", Georgia, "Times New Roman", serif';
...
function signOff(c, a, b, o = {}) { const { x = 540, y = 540, size = 60, sizeB = size * .75, ink = PAL.ink, ink2 = PAL.accents[0], progressA = 1, progressB = 1 } = o; const ta = a.slice(0, Math.round(a.length * progressA)), tb = b.slice(0, Math.round(b.length * progressB));
  handText(c, ta, x, y - 12, { size, ink, ink2, align: 'center' }); if (tb) handText(c, tb, x, y + size * 1.1, { size: sizeB, ink, ink2, align: 'center' }); seedDot(c, x - 10, y + size * .35, 6, ink, null); seedDot(c, x + 10, y + size * .35, 6, ink2, null); }
